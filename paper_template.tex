...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.44\textwidth]{project_architecture.png}
    \caption{Architectural Overview of Solution}
    \label{fig:project_architecture}
\end{figure}

A conceptual architecture of the proposed solution is presented in Figure \ref{fig:project_architecture}. In an ideal heterogeneous scenario, the leaf nodes remain device-agnostic, transmitting serialized data in a standardized format to the parent node. The parent node, acting as a load balancer, consolidates the data for transmission to the central server. The central server is tasked with handling computationally intensive processes such as data preprocessing, mining, and visualization. Due to time and material constraints, our initial focus will be on developing a single set of root-leaf nodes with one set of devices—a proof of concept to validate the proposed system.
